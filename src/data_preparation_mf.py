...
CUT_POINT = 1000
SAMPLE_CSV_FILE = "processed_mf.csv"

TARGET_CSV_FILE = "mf_first_level_ancestors.csv"
TARGET_DICT= "mf_first_level.txt"
TERMS_DICT = {}
...
def preprocess_data_set(old_df):
    ...
    for protien in protein_id_ls:
        #print("at present processing ID is ",protien)
        one_sample_ls=[]
...
def check_any_nan(data_frmae,specific_column) :
     
    print(data_frmae.isnull().any().any())

    null_columns=data_frmae.columns[data_frmae.isnull().any()]
    print(data_frmae[DATA[specific_column].isnull()][null_columns])       
